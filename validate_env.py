...
def validate_env():
    ...
    try:
        ...
        if not settings.api_key and not settings.api_keys:
            security_warnings.append("❌ API Key: Not Set (CRITICAL: Inference will fail)")
        ...
        if settings.hmac_enabled and not settings.hmac_secret:
            security_warnings.append("❌ HMAC Secret: Enabled but not set (CRITICAL: HMAC verification will fail)")
        ...
        print(f"   - Circuit Breaker: {'Enabled' if settings.breaker_enabled else 'Disabled'}")
