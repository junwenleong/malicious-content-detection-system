...
def test_hmac_enforcement(enable_hmac):
    with TestClient(app) as client:
        # 1. Missing Header
        resp = client.post("/v1/predict", json={"texts": ["foo"]}, headers={"x-api-key": "test-key"})
        assert resp.status_code == 401
        assert "Missing X-Signature" in resp.text

        # 2. Invalid Format
        resp = client.post("/v1/predict", json={"texts": ["foo"]}, headers={
            "x-api-key": "test-key",
            "X-Signature": "invalid"
        })
        assert resp.status_code == 401
        ...
        resp = client.post("/v1/predict", content=content, headers={
            "x-api-key": "test-key",
